...
def get_args_parser():
    ...
    parser.add_argument('--which', type = str, choices = ['A', 'O', 'R', 'C'])
...
# wandb.run.name = "imagenetc_baseline"
...
def get_imagenet_c_results(model, imagenet_c_tiny_mask=None, use_wandb = False):
    result_dict = {}
    ce_alexnet = utils_robust.get_ce_alexnet()
    ...
    for name, path in utils_robust.data_loaders_names.items():
        for severity in range(1, 6):
            inc_dataset = torchvision.datasets.ImageFolder(os.path.join(args.data_path, 'imagenet-c', path, str(severity)), transform=inc_transform)
    ...
    for name, path in utils_robust.data_loaders_names.items():
        ...
        acc_top1 /= 5
        CE = utils_robust.get_mce_from_accuracy(acc_top1, ce_alexnet[name])
        mCE += CE
    ...
    if use_wandb:
        wandb.log({'Corruption Top1 acc': overall_acc})
        wandb.log({'Corruption Top1 mCE': mCE*100,})

def main(args):

    from datasets import build_transform
    transformer_deit = build_transform(is_train= False, args = args)
    ...
    if args.use_wandb:
        wandb.init(project = 'SoftMoEImnet-C')
    ...
    PATH_TO_IMAGENET_R = os.path.join(args.data_path, 'imagenet-r')
    PATH_TO_IMAGENET_A = os.path.join(args.data_path, 'imagenet-a')
    PATH_TO_IMAGENET_O = os.path.join(args.data_path, 'imagenet-o')
    PATH_TO_IMAGENET_VAL = '/cm/shared/cuongdc10/datasets/image/val/'
    ...
    # import pandas as pd
    # names = pd.read_csv('Imagenet-C-labels.txt', header= None, delimiter= '  ')
    # imagenet_c_tiny_wnids = []
    # for i in range(12):
    #     for j in range(17):
    #         if names.iloc[i, j] is not None:
    #             imagenet_c_tiny_wnids.append(names.iloc[i, j])

    # imagenet_c_tiny_mask = [wnid in set(imagenet_c_tiny_wnids) for wnid in all_wnids]


    naes = dset.ImageFolder(root=PATH_TO_IMAGENET_A, transform=transformer_deit)
    nae_loader = torch.utils.data.DataLoader(naes, batch_size=64, shuffle=False,
                                         num_workers=4, pin_memory=True)
    

    nres = dset.ImageFolder(root=PATH_TO_IMAGENET_R, transform=transformer_deit)
    nre_loader = torch.utils.data.DataLoader(nres, batch_size=128, shuffle=False,
                                         num_workers=4, pin_memory=True)


    noes = dset.ImageFolder(root=PATH_TO_IMAGENET_O, transform=transformer_deit)
    noe_loader = torch.utils.data.DataLoader(noes, batch_size=64, shuffle=False,
                                            num_workers=4, pin_memory=True)

    imagenet_o_folder = os.path.join(args.data_path, 'imagenet_o_val/')
    ...
    def create_symlinks_to_imagenet(imagenet_folder, folder_to_scan):
        if not os.path.exists(imagenet_folder):
            ...
            for folder in folders_of_interest:
                rel_path_src = os.path.relpath(path_prefix + folder, os.path.dirname(imagenet_folder+folder))
                os.symlink(rel_path_src, imagenet_folder+folder, target_is_directory=True)

    create_symlinks_to_imagenet(imagenet_o_folder, PATH_TO_IMAGENET_O)

    val_examples_imagenet_o = dset.ImageFolder(root=imagenet_o_folder, transform=transformer_deit)
    val_loader_imagenet_o = torch.utils.data.DataLoader(val_examples_imagenet_o, batch_size=128, shuffle=False,
                                            num_workers=4, pin_memory=True)

    val_imagenet = dset.ImageFolder(root=PATH_TO_IMAGENET_VAL, transform=transformer_deit)
    ...
    # 2 iter/layer
    # model_path = '/root/checkpoints/13_01_2024_15:41:18_deit_tiny_patch16_224_checkpoint.pth'
    # finetune 2 iter/layer to 3 iter/layer
    # model_path = '/root/checkpoints/19_01_2024_09:22:01_deit_tiny_patch16_224_checkpoint.pth'
    # 4 iter in 1st layer
    # model_path = '/root/checkpoints/16_01_2024_08:38:32_deit_tiny_patch16_224_checkpoint.pth'
    # 4 iter in 1st layer to 5 iter
    # model_path = '/root/checkpoints/22_01_2024_16:01:10_deit_tiny_patch16_224_checkpoint.pth'
    # 4 iter in 1st layer to 5 iter to 6 iter
    # model_path = '/root/checkpoints/23_01_2024_06:28:38_deit_tiny_patch16_224_checkpoint.pth'
    # model_path = '/root/checkpoints/22_01_2024_23:56:51_deit_tiny_patch16_224_checkpoint.pth'
    # model_path = '/root/checkpoints/25_01_2024_14:19:08_deit_tiny_patch16_224_checkpoint.pth'
    # baseline
    # model_path = '/root/checkpoints/Sym_data/09_12_2023_20:58:31_deit_tiny_patch16_224_checkpoint.pth'
    # 4 iter in last layer
    # model_path = "/root/checkpoints/18_01_2024_08:46:56_deit_tiny_patch16_224_checkpoint.pth"
    ...
    if args.which == 'R':
        print("ImageNet-R Results")
        get_imagenet_a_results(nre_loader, net=model, mask=imagenet_r_mask)
    elif args.which == 'A':
        ...
        print('get_imagenet_c_results')
        get_imagenet_c_results(model, use_wandb=args.use_wandb)
