...
_C.MODEL.SWIN_MOE.USE_COSA = False
_C.MODEL.SWIN_MOE.MAD = True
_C.MODEL.SWIN_MOE.MIX_WEIGHTS = False
_C.MODEL.SWIN_MOE.COSA_POSITIONS = []
...
# -----------------------------------------------------------------------------
_C.WANDB = CN()
_C.WANDB.use_wandb = 1
_C.WANDB.project_name = None
_C.WANDB.job_name = None
...
def update_config(config, args):
    ...
    if _check_args('project_name'):
        config.WANDB.project_name = args.project_name
    if _check_args('job_name'):
        config.WANDB.job_name = args.job_name
