**This repo is adapted from the [Swin Transformer](https://arxiv.org/pdf/2103.14030) repo available [here](https://github.com/microsoft/Swin-Transformer)**

# Swin Transformer with Adaptive Clustering (ACMoE) for Image Classification

This folder contains the implementation of ACMoE in the Swin Transformer backbone for image classification under clean settings and under adversarial attack.
...
### Mixture-of-Experts Support - Install [Tutel](https://github.com/microsoft/tutel)
...
To evaluate a pre-trained model on ImageNet val, run:
...
For example, to evaluate Swin Base, `Swin-B`, with a single GPU:
...
To train Swin-MoE on ImageNet from scratch, run:

```bash
CUDA_VISIBLE_DEVICES=<gpus-selected> python -m torch.distributed.launch --nproc_per_node <num-of-gpus-to-use> --master_port 12345  --use_env main_moe.py \
--cfg configs/swinmoe/R_swin_moe_small_patch4_window7_192_16expert_4gpu_1k.yaml --data-path /path/to/imagenet \
--batch-size <batch-size> \
--output /path/to/output \
```

To then evaluate on adversarial attack, for example projected gradient descent with 5/255 perturbation budget, add

```bash
--attack 'pgd' --eps 5
```

These shell script is also found in [run_baseline.sh](run_baseline.sh).

To train ACMoE on ImageNet from scratch, run:

```bash
CUDA_VISIBLE_DEVICES=<gpus-selected> python -m torch.distributed.launch --nproc_per_node <num-of-gpus-to-use> --master_port 12345  --use_env main_moe.py \
--cfg configs/swincosa/R_swin_moe_small_patch4_window7_192_16expert_4gpu_1k.yaml --data-path /path/to/imagenet \
--batch-size <batch-size> \
--output /path/to/output \
```
These shell script is also found in [run_acmoe.sh](run_acmoe.sh).
...
Find the original README markdown file from the Swin repo at [README-swin.md](README-swin.md)