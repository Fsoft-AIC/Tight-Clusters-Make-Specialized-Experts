...
class MoEMlp(nn.Module):
    ...
    def forward(self, x):
        x = self._moe_layer(x)
        return x, x.l_aux, x.topk_values, x.topk_indices
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 cosine_router_dim=256, cosine_router_init_t=0.5, moe_drop=0.0, use_cosa=False, mad=True, mix_weights=False):
        super().__init__()
        self.dim = dim
        ...
        self.smooth_dist = torch.linspace(0.1, 0.9, 50).tolist()
        self.alpha_counter = 0
        self.load_counter = torch.zeros(16, dtype=torch.int32)
        self.router_stability = 0.0

    def forward(self, x, topk_values=None, topk_indices=None):
        H, W = self.input_resolution
        B, L, C = x.shape
        ...
        ######## COSA #########
        if self.use_cosa and topk_values is not None and self.is_moe: # remove self.is_moe just for cosa1 version
            w = self.compute_cosa(x, topk_values, topk_indices)
            w = w.view(B, H * W, C)
            x = w * x

            if False:
                self.counter += 1
                if self.counter % 2502 == 0:
                    self.epoch +=1
                    self.counter = 0
                    if self.epoch >= 45:
                        self.clamp_roof = 0.96*self.clamp_roof
                        self.clamp_floor = 1/self.clamp_roof
        ...
        #######################
        if self.is_moe:
            topk_previous = topk_indices
            
            x, l_aux, topk_values, topk_indices = self.mlp(x)
            #### router stability
            ...
            #     topk_expanded = topk_indices[:,0].unsqueeze(0)  # Shape: (1, n)
            #     topk_t_expanded = topk_indices[:,0].unsqueeze(1)  # Shape: (n, 1)
            ...
            ######## router stability
                #print(f'router stability inside model: {self.router_stability}')

            # # for computing load balance
            # flattened = topk_indices.flatten()
            # counts = torch.bincount(flattened, minlength=16)
            # self.load_counter = self.load_counter.to(counts.device) +  counts


            #### for tsne #####
            self.representations = x, topk_indices
            #### for tsne #####

            x = shortcut + self.drop_path(x)
            return x, l_aux, topk_values, topk_indices
        else:
            x = shortcut + self.drop_path(self.mlp(x))
            return x
    
    def compute_cosa(self, input, mixing_weights, cluster_assignments, reserve_dims=1):
        original_shape, original_dtype  = input.shape, input.dtype

        tokens = input.reshape(-1, original_shape[-reserve_dims:].numel())
        self.top_k = cluster_assignments.shape[1]
        with torch.no_grad():
            ...
            if self.mad: # mean absolute deviation
                ...
                #print('here var')
            #self.W = W.min(), W.max(), W.mean(), W.std(), self.clamp_roof, alpha
            # print('here cosa')
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 cosa_block = [-1], mad=True, mix_weights=False):
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 use_cosa= True if i in cosa_block else False, mad=mad, mix_weights=mix_weights,
                                 )
            for i in range(depth)])
        self.use_cosa = True if -1 not in cosa_block else False
    ...
    def forward(self, x, topk_values=None, topk_indices=None):
        ...
        for ind, blk in enumerate(self.blocks):
            if self.use_checkpoint:
                ...
                out = blk(x, topk_values=topk_values, topk_indices=topk_indices)
...
class SwinTransformerMoECOSA(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_cosa=False, mad=True, cosa_blocks = [[-1], [-1], [-1], [-1]], mix_weights=False, **kwargs):
        super().__init__()
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               cosine_router_init_t=cosine_router_init_t,
                               moe_drop=moe_drop, 
                               cosa_block = cosa_blocks[i_layer], mad = mad, mix_weights=mix_weights
                               #use_cosa=use_cosa if depths[i_layer]==max_depth else False, mad=mad, mix_weights=mix_weights,
                               )
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            topk_values = None
            topk_indices = None
            x, cur_l_aux, topk_values, topk_indices = layer(x, topk_values=topk_values, topk_indices=topk_indices)
            l_aux = cur_l_aux + l_aux
